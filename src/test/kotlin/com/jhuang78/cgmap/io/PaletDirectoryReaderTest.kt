...
class PaletDirectoryReaderTest : Spek({

	describe("a PaletDirectoryReader") {

		given("an valid directory") {
			val reader = PaletDirectoryReader(getPathForResource("palet"))

			it("should list palets") {
				assertThat(reader.palets()).containsExactly(Paths.get("palet_00.cgp"))
			}

			given("an valid palet file") {
				val palet = reader.read(Paths.get("palet_00.cgp"))
