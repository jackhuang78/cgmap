...
class GraphicFileReader(val path: Path) : AutoCloseable {
	...
	fun read(
		...
		try {
			...
			logger.error(
					e,
					{ "The read Graphic is probably corrupted" })
		}
...
fun Graphic.validate(): Graphic {
	checkState(
			magic == GRAPHIC_ENTRY_MAGIC,
			"Expect magic to be ${GRAPHIC_ENTRY_MAGIC} (\"RD\"), but got ${magic}")
	checkState(
			width > 0,
			"Expect width to be positive, but got ${width}");
	checkState(
			height > 0,
			"Expect height to be positive, but got ${height}");
	checkState(
			dataLength == GRAPHIC_ENTRY_HEADER_SIZE + data.capacity(),
