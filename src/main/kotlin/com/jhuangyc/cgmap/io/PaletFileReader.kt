...
@Deprecated("Use PaletFileReader.read() instead")
fun readPaletFile(file: Path): Palet {

	FileChannel.open(file, StandardOpenOption.READ).use {
		val buffer = it.map(FileChannel.MapMode.READ_ONLY, 0,
				PaletFileReader.PALET_FILE_SIZE);

		return Palet(colors = List(PaletFileReader.PALET_NUM_COLORS) {
			when (it) {
